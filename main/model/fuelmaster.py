...
# Fuelmaster class inherits from db.Model class which declares the class as a model for sqlalchemy.
class Fuelmaster(db.Model):
    """ Fuelmaster Model for storing Fuelmaster related details """
    __tablename__ = "fuelmaster"
    
    # line 18 through 21 creates the required columns for the Fuelmaster table.
    fuelid = db.Column(db.Integer, primary_key=True, nullable=False,autoincrement=True)
    wheels = db.Column(db.Integer)
    fuelpermt = db.Column(db.Integer)
    effectivefrom = db.Column(db.DateTime, default=datetime.datetime.now())
...
#compares a given password with already savedpassword_hash.
    ...
    def __repr__(self):
        return "<fuelmaster '{}'>".format(self.wheels)
