...
# Roles class inherits from db.Model class which declares the class as a model for sqlalchemy.
class Roles(db.Model):
    """ Users Model for storing Roles related details """
    __tablename__ = "Roles"
    
    # line 18 through 19 creates the required columns for the Roles table.
    RoleId = db.Column(db.Integer, primary_key=True, nullable=False,autoincrement=True)
...
#compares a given password with already savedpassword_hash.
    ...
    def __repr__(self):
        return "<Roles '{}'>".format(self.RoleName)

# Encoding tokens
def encode_auth_token(self, RoleId):
        """
